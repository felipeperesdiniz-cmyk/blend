            ...
            <AnimatedSection delay={0.25}>
              <div className="service-detail__body">
                ...
                {sd.faq && sd.faq.length > 0 && (
                  <a href="#faq" className="service-faq-prompt">
                    {t.faqPrompt}
                  </a>
                )}
                          ...
                          {(group.items as PriceItem[]).map((item) => (
                            ...
                            >
                              <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem' }}>
                                <span style={{ fontSize: '1rem', color: 'var(--text-2)', fontWeight: 300 }}>{item.label}</span>
                                <span style={{ fontSize: '1rem', fontWeight: 500, whiteSpace: 'nowrap' }}>{item.price}</span>
                              </div>
                              {item.note && (
                                <p style={{ fontSize: '0.88rem', color: 'var(--text-3)', fontWeight: 300, lineHeight: 1.6, marginTop: '0.4rem' }}>
                                  {item.note}
                                </p>
                              )}
                            </div>
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', marginTop: '2rem' }}>
                  ...
                  {!sd.priceGroups && (
                    <span style={{ fontSize: '0.85rem', color: 'var(--text-3)', fontStyle: 'italic', fontFamily: 'var(--serif)' }}>
                      {sd.priceNote}
                    </span>
                  )}
                </div>
